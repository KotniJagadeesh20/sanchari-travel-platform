...
class RideBookingServiceImplTest {
    ...
    class BookRide {
        ...
        @Test
        void reservesAvailableSeats_atPendingTime() {
            when(rideRepo.findForUpdateById(rideId)).thenReturn(Optional.of(ride));
            when(bookingRepo.save(any(RideBooking.class))).thenAnswer(i -> i.getArgument(0));

            bookingService.bookRide(rideId, 2, passenger);

            assertEquals(2, ride.getAvailableSeats(), "Pending bookings reserve seats immediately");
    ...
    class ApproveBooking {
        ...
        void approvesAlreadyReservedBooking_whenCallerIsDriver() {
            ...
            assertEquals(4, ride.getAvailableSeats(), "Approval does not deduct already-reserved seats again");
        }
        ...
        void throwsBookingNotFound_whenBookingDoesNotExist() {
            when(bookingRepo.findForUpdateById(bookingId)).thenReturn(Optional.empty());
