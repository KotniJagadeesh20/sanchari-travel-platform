 ...
 * ride-share-service, and travel-packages-service. This service does not own user
 * accounts — that's auth-service's job. We persist only the UUID + email/name
 * (synced from JWT claims forwarded by the gateway) so Hotel.createdBy can have a
 * real FK without a cross-service call.
 */
@Entity
@Table(name = "user_ref")
