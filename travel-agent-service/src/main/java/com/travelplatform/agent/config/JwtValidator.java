...
/**
 * Populates the SecurityContext from identity headers forwarded by the API
 * Gateway (X-Authenticated-Email, X-Authenticated-Authorities), rather than
 * re-parsing the JWT. Identical pattern to every other downstream service
 * (see bus-booking-service's JwtValidator for the full rationale and the
 * network-isolation requirement this depends on).
 */
